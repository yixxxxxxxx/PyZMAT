...
# MP2 GRADIENT header and lines
MP2_GRAD_LINE_RE = re.compile(
    rf'^\s*({_INT})\s*:\s*({_FLOAT})\s+({_FLOAT})\s+({_FLOAT})\s*$'
)
MP2_TITLE_RE = re.compile(r'^\s*The\s+final\s+MP2\s+gradient\s*$', re.IGNORECASE)
...
class ParseUtils:
    ...
    def _parse_forces(lines: List[str]) -> List[float]:
        ...
        if headers:
            ...
            # MP2 block starts immediately after the title, skip blank lines
            start = mp2_header + 1
            ...
            rows = []
            for L in lines[start:]:
                m = MP2_GRAD_LINE_RE.match(L)
                if m:
                    idx = int(m.group(1))
                    fx = float(m.group(2)); fy = float(m.group(3)); fz = float(m.group(4))
                    rows.append((idx, fx, fy, fz))
                    continue
                # Stop on blank line or anything non-matching after matching at least one
                if rows and (L.strip() == "" or not MP2_GRAD_LINE_RE.match(L)):
                    break

            if not rows:
                raise ValueError("MP2 gradient header found, but no gradient rows parsed.")

        # ----------------------------
        # 3. Finish: sort + unit conversion + ORCA sign fix
        # ----------------------------
        rows.sort(key=lambda t: t[0])
        ...
        forces = []
        R = np.diag([1, -1, -1])   # compensate ORCA Y/Z sign flip
